...
export function EditCompanyModal({ isOpen, onClose, onSuccess, initialData }: EditCompanyModalProps) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setInvoiceNumberPrefix(initialData.invoice_number_prefix || `INV-${new Date().getFullYear()}-`);
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit}>
            ...
            {tab === "company" && (
              <>
                <div className="flex gap-3 items-start">
                  <div className="flex-1 space-y-3">
                    ...
                    <fieldset className={`${fs} ${!email.trim() ? 'border-red-400/60 dark:border-red-500/40' : ''}`}>
                      <legend className={`${lg} ${!email.trim() ? 'text-red-500' : ''}`}>{t.billingEmailField} <span className="text-red-500">*</span></legend>
                ...
                <fieldset className={`${fs} ${!address.trim() ? 'border-red-400/60 dark:border-red-500/40' : ''}`}>
                  <legend className={`${lg} ${!address.trim() ? 'text-red-500' : ''}`}>{t.addressField} <span className="text-red-500">*</span></legend>
                ...
                <div>
                  ...
                  {signatureUrl ? (
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => document.getElementById('sig-upload-edit')?.click()} className="flex-1 h-20 rounded-xl border-2 border-dashed border-zinc-300 dark:border-zinc-700 hover:border-blue-500 flex flex-col items-center justify-center gap-0.5 cursor-pointer transition-colors text-zinc-400 hover:text-blue-500">
          ...
          <div className="px-5 pb-4 pt-3 flex items-center justify-end gap-4 border-t border-zinc-100 dark:border-zinc-800">
            ...
            <button type="submit" disabled={isSubmitting || !name.trim() || !email.trim() || !address.trim()} className="inline-flex items-center gap-2 px-5 py-2 text-sm font-semibold text-white bg-blue-500 hover:bg-blue-600 rounded-xl transition-colors disabled:opacity-50 shadow-sm">
