...
import { updateItem } from "@/utils/supabase/items-actions";
...
interface EditItemModalProps {
  isOpen: boolean;
  initialData: SavedItem;
  onClose: () => void;
  onSuccess: (updatedItem: any) => void;
}

export function EditItemModal({ isOpen, initialData, onClose, onSuccess }: EditItemModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedItem = await updateItem(session.access_token, initialData.id, name, Number(rate) || 0);
