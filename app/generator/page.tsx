...
import { Download, Save, Building2, X, Plus, Receipt } from "lucide-react";
        ...
        <div className="flex flex-col xl:flex-row gap-8 pb-32 xl:pb-20">
          ...
          <div className="w-full xl:w-1/2 flex flex-col gap-6">
            <div className="flex items-center justify-between mb-2">
              <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Editor</h1>
            </div>
      ...
      {isGuestSaveModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden ring-1 ring-zinc-200 dark:ring-zinc-800 animate-in zoom-in-95 duration-200 p-8 text-center relative">
            <button onClick={() => setIsGuestSaveModalOpen(false)} className="absolute top-4 right-4 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="space-y-3">
              <Link href="/login?redirect=/generator" className="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                Create Free Account
              </Link>
              <Link href="/login?redirect=/generator" className="w-full flex justify-center py-3 px-4 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                Sign In
              </Link>
            </div>
          </div>
